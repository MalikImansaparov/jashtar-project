        ...
        <div className="w-full flex flex-row flex-wrap justify-center">
          {response &&
            response.map((item) => (
              <div key={item.id}>
                <div
                  className="w-[192px] h-[218px] mb-8 cursor-pointer transition-all hover:scale-125 mx-[22px] hover:scale-110 2lg:mx-[12px]"
                  onClick={() => openModal(item.id)}
                >
                  <div
                    className="w-full h-[160px] rounded shadow-partner flex justify-center items-center mb-[10px] cursor-pointer shadow-lg"
                    key={item.id}
                  >
                    <div className="h-[140px] max-w-[140px] overflow-hidden flex justify-center">
                      <img
